...
def build_order_lines(data: dict) -> tuple[list[str], dict]:
    ...
    if isinstance(raw_items, list) and raw_items:
        for it in raw_items:
            ...
            # красиво: если есть сумма/цена — показываем
...
async def webapp_data(message: types.Message):
    ...
    # ✅ собираем позиции устойчиво
    lines, normalized_order = build_order_lines(data)
    ...
    # ✅ если total есть, а позиций нет — добавим диагностику (без удаления логики)
    if ("⚠️ Корзина пустая" in lines) and (total_num > 0 or total_str.strip() not in ("", "0", "—")):
        keys = ", ".join(sorted(list(data.keys()))[:30])
        admin_text += (
            "\n\n⚠️ <b>ВНИМАНИЕ:</b> пришла сумма, но список блюд пуст.\n"
            f"🔎 Ключи payload: <code>{keys}</code>\n"
            "Проверь, что WebApp отправляет поле <code>order</code> или <code>items</code>."
        )
